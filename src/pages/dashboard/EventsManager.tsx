...
const EventsManager = () => {
  ...
  const performDelete = useCallback(async (id: string) => { await supabase.from("events").delete().eq("id", id); fetchItems(); }, [user]);
  const { requestDelete, DeleteConfirmDialog } = useDeleteConfirmation(performDelete, { title: "Delete this event?", description: "This event will be permanently removed from your portfolio." });
